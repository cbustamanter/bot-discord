...
client.on('message', (message) => {
    if (message.content === `${config_json_1.prefix}nosfe`) {
        ...
        if (time > 1 && time < 4) {
            message.channel.send('Es la hora del Nosfe, ejecuta !!callnosfe para llamarlo.').then(() => {
                message.channel.send(functions_1.Attachment('https://www.cinemascomics.com/wp-content/uploads/2019/10/nosferatu-1922-reboot.jpg')).then(value => value.react('📟'));
            });
        }
        else {
    ...
    if (message.content === `${config_json_1.prefix}callnosfe`) {
        message.channel.send(`Despierta Nosfe 🧛 <@314533506275606529>`).then(() => {
            message.channel.send(functions_1.Attachment('http://www.gatropolis.com/wp-content/uploads/2017/08/Remake-Nosferatu.jpg')).then(value => value.react('🧛'));
        });
    }
    ...
    if (message.content === `${config_json_1.prefix}ajo`) {
        message.channel.send(`Hiriendo al nosfe... \n🧄🧄🧄🧄🧄`).then(() => message.channel.send(functions_1.Attachment('https://i.pinimg.com/originals/6a/43/b1/6a43b14ee7b3b10695bc9ad373305432.gif')).then(value => value.react('🧄')));
    }
    ...
    if (message.content === `${config_json_1.prefix}profe`) {
        message.channel.send(`<@560481541986189348> CHAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA`).then(() => {
            message.channel.send(functions_1.Attachment('https://i0.wp.com/www.thatmomentin.com/wp-content/uploads/2015/08/bubbl-1.jpg')).then(value => value.react('🔥'));
        });
    }
