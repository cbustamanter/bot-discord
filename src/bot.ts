...
client.on("message", (message: Message) => {
  if (message.content === `${prefix}nosfe`) {
    ...
    if (time > 1 && time < 4) {
      message.channel
        ...
        .then(() => {
          message.channel
            .send(
              Attachment(
                "https://www.cinemascomics.com/wp-content/uploads/2019/10/nosferatu-1922-reboot.jpg"
              )
            )
      ...
      message.channel
        .send(
          "Es muy posible que a esta hora Nosfe se encuentre durmiendo, ejecuta !!nosfetime para consultar su horario."
        )
        .then((value) => value.react("⚰"));
    }
  }
    ...
    message.channel
      ...
      .then(() => {
        message.channel
          .send(
            Attachment(
              "http://www.gatropolis.com/wp-content/uploads/2017/08/Remake-Nosferatu.jpg"
            )
          )
          .then((value) => value.react("🧛"));
      });
    ...
    message.channel
      .send(
        "💁🏻 ***Comandos actuales:*** \n" +
          ...
          " **??ajo:** Ataca a Nosfe 🧄 \n" +
          " **??play :** Reproduce audio. Audios disponibles: camarones, dura, medejaronsolo, nena, proferisa, toyready, piensachato, atrapada " 
      )
    ...
    message.channel
      ...
      .then(() =>
        message.channel
          .send(
            Attachment(
              "https://i.pinimg.com/originals/6a/43/b1/6a43b14ee7b3b10695bc9ad373305432.gif"
            )
          )
          .then((value) => value.react("🧄"))
      );
    ...
    message.channel
      ...
      .then(() => {
        message.channel
          .send(
            Attachment(
              "https://i0.wp.com/www.thatmomentin.com/wp-content/uploads/2015/08/bubbl-1.jpg"
            )
          )
          .then((value) => value.react("🔥"));
      });
  ...
  if (message.content === `${prefix}estaca`) {
    message.channel.send(`Estás muy cerca de asesinar a nosfe`).then(() => {
      message.channel
        .send(
          "https://static.tvtropes.org/pmwiki/pub/images/dracula-1_6352.jpg"
        )
        .then((value) => value.react("🩸"));
    });
  }
